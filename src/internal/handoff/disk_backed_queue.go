...
func (l *DiskBackedQueue) loadSegments() (segments, error) {
	...
	for _, segment := range files {
		...
		segmentPath := filepath.Join(l.dir, segment.Name())
		segment, err := newSegment(segmentPath, l.maxSegmentSize)
		if err != nil {
			log.Printf("error opening hinted handoff segment %s: %s", segmentPath, err)
			err := os.Remove(segmentPath)
			if err != nil {
				log.Printf("couldn't remove hinted handoff file: %s", segmentPath)
			}
...
func (l *segment) append(b []byte) error {
	...
	blockPlusFooter := make([]byte, 0, 8+len(b)+8)
	length := make([]byte, 8)
	position := make([]byte, 8)
	binary.BigEndian.PutUint64(length, uint64(len(b)))
	binary.BigEndian.PutUint64(position, uint64(l.pos))
	blockPlusFooter = append(blockPlusFooter, length...)
	blockPlusFooter = append(blockPlusFooter, b...)
	blockPlusFooter = append(blockPlusFooter, position...)

	if err := l.writeBytes(blockPlusFooter); err != nil {
		return err
	}
...
func (l *segment) current() ([]byte, error) {
	...
	if int64(sz) > 512*1024 {
		return nil, fmt.Errorf("record size is almost certainly too big to be valid: max 512k: got %d", sz)
	}
