...
var _ = Describe("Histogram Rollup", func() {
	...
	It("returns histograms even when no new metrics were rolled up recently", func() {
		rollup := NewHistogramRollup(
			logger.NewTestLogger(GinkgoWriter),
			"0",
			[]string{"included-tag"},
	...
	It("expires histogram after no new metrics were seen for a given amount of time", func() {
		rollup := NewHistogramRollup(
			logger.NewTestLogger(GinkgoWriter),
			"0",
			[]string{"included-tag"},
			WithHistogramRollupExpiration(2*time.Nanosecond),
		)
		...
		histograms = extract(rollup.Rollup(2))
		Expect(histograms).To(BeEmpty())
	})
})
