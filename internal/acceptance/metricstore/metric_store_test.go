...
import (
	...
	"github.com/cloudfoundry/metric-store/pkg/metricstore"
	...
	. "github.com/cloudfoundry/metric-store/pkg/matchers"
...
var _ = Describe("MetricStore", func() {
	...
	var start = func(tc *testContext) {
		...
		tc.metricStoreProcess = testing.StartGoProcess(
			"github.com/cloudfoundry/metric-store/cmd/metric-store",
			[]string{
				"ADDR=" + tc.addr,
		...
		tc.gatewayProcess = testing.StartGoProcess(
			"github.com/cloudfoundry/metric-store/cmd/gateway",
			[]string{
				"ADDR=" + tc.gatewayAddr,
